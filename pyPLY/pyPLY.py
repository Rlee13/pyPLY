...
class Lamina:
    ...
    def define(self, name, mat_no_name, angle):
        ...
        if isinstance(mat_no_name, int):
            self.mat_no = mat_no_name
        ...
        for k, v in lam.iteritems():
            if (v == self.mat_no):
                ...
                try:
                    ...
                    self.alpha_11 = k.alpha_11;
                    self.alpham[0] = k.alpha_11
                    self.alpha_22 = k.alpha_22;
                    self.alpham[1] = k.alpha_22
                    self.alpham[2] = 0.0
                ...
                try:
                    ...
                    self.beta_11 = k.beta_11;
                    self.betam[0] = k.beta_11
                    self.beta_22 = k.beta_22;
                    self.betam[1] = k.beta_22
                    self.betam[2] = 0.0
        ...
        for i in range(0, 3):
            for j in range(0, 3):
                for k in range(0, noPly):
                    ...
                    self.B[i, j] = self.B[i, j] + ((self.ZStack[k + 1] - (self.THK) / 2) ** 2 - (
                    self.ZStack[k] - (self.THK) / 2) ** 2) * qtemp[i, j] / 2.0
                    self.D[i, j] = self.D[i, j] + ((self.ZStack[k + 1] - (self.THK) / 2) ** 3 - (
                    self.ZStack[k] - (self.THK) / 2) ** 3) * qtemp[i, j] / 3.0
                    # ************************************************************************
...
class Loading():
    ...
    def apply_To(self, laminate):
        ...
        if (self.deltaT):
            for k in range(0, noPly):
                self.thermal_load[0:3] = self.thermal_load[0:3] + self.laminate_name.QU[k] * \
                                                                  self.deltaT * self.laminate_name.ply_list[
                                                                      k].alphas_xy * \
                                                                  (self.laminate_name.ZStack[k + 1] -
                                                                   self.laminate_name.ZStack[k])
                self.thermal_load[3:6] = self.thermal_load[3:6] + self.laminate_name.QU[k] * \
                                                                  (self.deltaT * 0.5) * self.laminate_name.ply_list[
                                                                      k].alphas_xy * \
                                                                  ((self.laminate_name.ZStack[
                                                                        k + 1] - self.laminate_name.THK / 2) ** 2 - \
                                                                   (self.laminate_name.ZStack[
                                                                        k] - self.laminate_name.THK / 2) ** 2)
        ...
        if (self.deltaM):
            for k in range(0, noPly):
                self.moisture_load[0:3] = self.moisture_load[0:3] + self.laminate_name.QU[k] * \
                                                                    self.deltaM * self.laminate_name.ply_list[
                                                                        k].betas_xy * \
                                                                    (self.laminate_name.ZStack[k + 1] -
                                                                     self.laminate_name.ZStack[k])
                self.moisture_load[3:6] = self.moisture_load[3:6] + self.laminate_name.QU[k] * \
                                                                    (self.deltaM * 0.5) * self.laminate_name.ply_list[
                                                                        k].betas_xy * \
                                                                    ((self.laminate_name.ZStack[
                                                                          k + 1] - self.laminate_name.THK / 2) ** 2 - \
                                                                     (self.laminate_name.ZStack[
        ...
        for k in range(0, noPly):
            ...
            stress_ply = self.laminate_name.QU[k] * (
            strain_ply - self.deltaT * self.laminate_name.ply_list[k].alphas_xy - \
            ...
            strain_ply = self.epsilon_K[0:3] + (-self.laminate_name.THK / 2 +
                                                self.laminate_name.ZStack[k] + (
                                                self.laminate_name.ZStack[k + 1] - self.laminate_name.ZStack[
                                                    k]) / 2.0) * \
                                               self.epsilon_K[3:6]
            strain_ply12 = laminate.ply_list[k].Teps * strain_ply
            self.list_ply_strains_xy.append(strain_ply)
    ...
    def change_Load(self, Nx, Ny, Nz, Mx, My, Mz, deltaT=0.0, deltaM=0.0, loadType=0):
        ...
        try:
            ...
            self.Nz = float(Nz);
            self.load[2] = float(Nz)
            self.Mx = float(Mx);
            self.load[3] = float(Mx)
            self.My = float(My);
            self.load[4] = float(My)
            self.Mz = float(Mz);
            self.load[5] = float(Mz)
        except:
            exit(errorString[4])
